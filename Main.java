...
class Main{
    ...
    public static List<String> printWords(int[] numbers,int len,int numIndex,String s){
        if(len == numIndex) {
            return new ArrayList<>(Collection.singleton(s));
        }
        List<String> stringList = new ArrayList<>();
        for(int == 0;i<numberToCharMap[numbers[numIndex]].length;i++){
